...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        $image_file = '../../vistas/img/LOGO_RANSA.png';
...
$rptaListCat = ControladorProductosHolcim::ctrConsultarProductosHolcim($_SESSION["idcliente"]);
...
if ($rptaListCat) {
	...
	for ($i=0; $i < count($rptaListCat) ; $i++) {
		...
		$tbl .= '<tr>
			...
			<td>'.$rptaListCat[$i]["familia"].' / '.$rptaListCat[$i]["grupo"].'</td>
			<td>'.$rptaListCat[$i]["descripcion"].'</td>';

		if (!empty($rptaListCat[$i]["foto_portada"])) {

			$imagen = '../../ransa/'.$rptaListCat[$i]["foto_portada"];
		$tbl .= '<td><img src="'.$imagen.'" border="0" height="90" width="140" /></td>';
